...
// //v3 no helper function - bad - utilization 77%
// #include <iostream>
// #include "typedefs.h"
// #include "sha256.h"
...
//     bit32_t H[32][8];
//     #pragma HLS array_partition variable=H factor=4 dim=2

//     // Initialize message array
//     for (int i = 0; i < bitlength / 32 + 1; i++) {
//         #pragma HLS unroll
//         message[i] = input[i];
//     }

//     if (bitlength % 32 != 0)
//         message[bitlength / 32] |= (1 << (32 - 1 - bitlength % 32));
//     else
//         message[bitlength / 32] = 1 << 31;

//     // Compute rounds
//     int rounds = (bitlength / 32 + 16 - 1) / 16 + 1;
//     for (int i = 0; i < 16; i++) {
//         #pragma HLS pipeline II=1
//         for (int j = 0; j < rounds; j++) {
//             #pragma HLS unroll
...
//         bit32_t W[64];
//         #pragma HLS array_partition variable=W block factor = 32 dim=1
...
//             bit32_t T1 = h + Sig1f(e) + Ch(e, f, g) + K[j] + W[j];
//             bit32_t T2 = Sig0f(a) + Maj(a, b, c);
//             h = g;
//             g = f;
//             f = e;
//             e = d + T1;
...
void prepareMessage(bit32_t *input, int bitlength, bit32_t M[32][16]) {
    ...
    // Add bitlength information
    if (wordlength % 16 == 0 || wordlength % 16 == 15)
        message[wordlength + 15 + 16 - wordlength % 16] = bitlength;
    else
        message[wordlength + 15 - wordlength % 16] = bitlength;

    // Populate M from message
    for (int j = 0; j < (bitlength / 512 + 1); j++) {
        #pragma HLS pipeline II=1
        for (int i = 0; i < 16; i++) {
            #pragma HLS unroll
...
void computeHashRound(bit32_t H_prev[8], bit32_t M[16], bit32_t K[64], bit32_t H_new[8]) {
    ...
    #pragma HLS array_partition variable=H_new complete
    #pragma HLS array_partition variable=M complete
